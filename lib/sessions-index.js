...
function parseHead(file) {
  ...
  for (const line of readChunk(file, 0, HEAD_BYTES).split('\n')) {
    try {
      ...
      if (o.type === 'user' && o.message?.role === 'user' && !o.isMeta && !o.isSidechain) {
...
function parseTail(file, size) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    for (let j = arr.length - 1; j >= 0; j--) {
      const it = arr[j];
      if (it.kind === 'msg' && it.role !== 'tool' && it.text) {
...
function deepScan(file, size) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (!cwd && o.cwd) cwd = o.cwd;
    if (!firstUserUuid && o.type === 'user' && o.message?.role === 'user' && !o.isMeta && !o.isSidechain) {
