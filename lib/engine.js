...
const { parseFileSync, sessionFileFor } = require('./transcript');
...
class HeadlessSession extends EventEmitter {
  ...
  spawn() {
    ...
    // 부모가 Claude Code 세션이면 CLAUDE* env가 상속돼 transcript 저장이 꺼짐 → 제거
    ...
    // zsh 로그인+인터랙티브 셸 경유: fnm PATH 로딩 때문
    this.child = spawn('/bin/zsh', ['-ilc', cmd], {
      cwd: this.room.dir,
      env,
  ...
  _onEvent(line) {
    ...
    if (ev.type === 'system' && ev.subtype === 'init') {
      ...
      this.emit('info', { model: this._lastModel || ev.model || null, slashCommands: ev.slash_commands || [] });
    ...
    if (ev.type === 'assistant' && Array.isArray(ev.message?.content)) {
      ...
      for (const c of ev.message.content) {
        if (c.type === 'text' && c.text && c.text.trim()) {
          ...
          this.emit('message', {
            role: 'tool',
            text: `${c.name} · ${String(detail).slice(0, 5000)}`,
            ts,
          });
        }
      }
