...
const { wrapEnvelope } = require('./mentions');

// 자동 압축 임계 기본값 (토큰). 모델 한도와 무관한 절대값 — 이유는 _compactAt() 주석 참고.
// 방마다 room.compactAt으로 덮어쓸 수 있다 (COMPACT_CHOICES 안의 값만).
//
// 'limit' = 모델 한도가 찰 때까지 (LIMIT_RATIO 참고). 숫자면 그 절대값.
//
// 절대값 180k에서 'limit'으로 바꿨다. 근거는 실측 두 가지다.
// (1) 압축 기록 37건에서 소요시간 중앙값이 171초인데 크기와 거의 무관하다 (163k→238초,
//     807k→191초). 압축 한 번 값이 고정이면 횟수를 줄이는 게 그대로 이득이다.
// (2) 180k는 "TUI가 한도의 92%에서 압축한다"는 근거 없는 값에서 왔다. compact_boundary의
//     trigger=auto 기록을 보면 1M 모델에서 auto가 걸린 지점이 pre=1,000k — 사실상 한도다.
// 대가는 분명하다. 요청당 다시 읽는 양이 그만큼 커지고, 임계가 800k였던 8/29~8/30에는
// 요청당 평균 컨텍스트가 757k·891k, 캐시 읽기가 하루 비용의 78%였다. 캐시 읽기는 정가의
// 0.1배라 3분짜리 압축을 몇 번 더 맞는 것과 저울질할 만하다고 보고 사용자가 이 쪽을 골랐다.
// 잠든 방을 깨우는 값이 같이 커지는 건 compactBeforeIdle()이 막는다 — 내리기 전에 압축해서
// 재우므로 임계와 무관하게 깨우기는 바닥값이다.
const AUTO_COMPACT_AT = 'limit';

// 'limit'을 골랐을 때 실제로 압축을 거는 지점 = 한도의 92%.
// 한도를 꽉 채운 뒤에는 /compact 자체가 "Prompt is too long"으로 실패해서 방이 통째로
// 막힌다 — 압축도 턴이라 컨텍스트가 들어갈 자리가 있어야 한다. 8%를 남기는 이유다.
const LIMIT_RATIO = 0.92;

// 방별로 고를 수 있는 임계. 비용은 "요청 수 × 평균 컨텍스트"고 평균은 대략 (임계+바닥)/2라,
// 임계를 올리면 압축 횟수는 줄지만 매 요청이 그만큼 비싸진다. 어느 쪽이 이득인지는 방마다
// 다르다 — 짧게 묻고 끝나는 방과 며칠씩 파고드는 방의 답이 같을 수 없어서 선택지로 뒀다.
// 낮은 눈금을 남겨둔 이유: 도구를 많이 부르는 방은 턴 하나가 요청 수십 건이고 그 전부가
// 컨텍스트 전량을 다시 읽는다. 그런 방은 압축을 자주 맞더라도 낮게 두는 편이 싸다.
// 절대값은 _compactAt()에서 한도의 80%로 깎인다 — 200k 모델에서 400k를 고르면 압축이
// 걸리기 전에 "Prompt is too long"으로 방이 막히기 때문이다. 'limit'만 그 위로 간다.
const COMPACT_CHOICES = [140_000, 180_000, 250_000, 400_000, 'limit'];
...
class HeadlessSession extends EventEmitter {
  ...
  spawn() {
    ...
    this.child.on('exit', (code) => {
      ...
      if (this.queue?.length && code === 0) {
        ...
        this._writeUser(next.text, next.images, { emitMessage: false, ...next.opts });
      }
  ...
  _onEvent(line) {
    ...
    if (ev.type === 'assistant' && Array.isArray(ev.message?.content)) {
      ...
      if (u) {
        ...
        if (total > 0) {
          ...
          // 압축 직후 첫 요청의 크기 = 이 방의 "바닥"(시스템 프롬프트 + 툴 정의 + CLAUDE.md
          // + 요약). 실측하면 5만 안팎이라 결코 0이 아니다. 다음 압축 때 이 값으로 되돌려야
          // 상태줄이 한동안 5만 낮게 나오지 않는다.
          if (this._awaitFloor) {
            this._awaitFloor = false;
            this._floorTokens = total;
          }
        }
      }
    ...
    if (ev.type === 'result') {
      ...
      if (this._autoCompacting) {
        ...
        if (!ev.is_error) {
          // 0이 아니라 지난번에 잰 바닥으로 되돌린다 (아직 잰 적이 없으면 다음 요청이 알려준다)
          this.contextTokens = this._floorTokens || 0;
          this._awaitFloor = true;
  ...
  // 대기줄에서 아직 투입되지 않은 메시지 하나를 뺀다. 반환값은 실제로 뺀 자리(없으면 -1).
  //
  // 인덱스만 믿으면 안 되는 이유: 취소를 누른 순간과 이 호출이 도착하는 순간 사이에 압축이
  // 끝나 앞의 것이 이미 투입됐을 수 있다. 그러면 같은 자리에 다른 말이 앉아 있어서,
  // 인덱스만 보고 지우면 사용자가 지목하지 않은 말이 사라진다. 본문으로 한 번 더 맞춘다.
  //
  // 이미 stdin으로 나간 말은 여기서 다루지 않는다 — 그건 취소가 아니라 interrupt다.
  cancelQueued(index, text = null) {
    const q = this.queue;
    if (!q?.length) return -1;
    const ok = (i) => i >= 0 && i < q.length && (text == null || q[i].text === text);
    const at = ok(index) ? index : q.findIndex((e) => e.text === text);
    if (at < 0) return -1;
    q.splice(at, 1);
    return at;
  }

  // 사용자·전달 메시지를 UI 말풍선 형태로 (전달돼 온 말은 내 말풍선이 아니다)
  _bubble(text, images, opts = {}, extra = {}) {
    return {
      role: opts.peerFrom ? 'peer' : 'user',
      from: opts.peerFrom || undefined,
      text,
  ...
  // opts.peerFrom  — 다른 방에서 전달돼 온 말. 봉투를 씌워 보내고 말풍선도 그쪽 이름으로.
  // opts.wireSuffix — stdin에만 덧붙일 안내 (말풍선에는 안 보인다).
  send(text, images = [], opts = {}) {
    if (!this.child) this.spawn();
    // spawn이 실패하면(폴더가 사라진 방 등) child가 없다. 그대로 진행하면 stdin.write에서
    // 메인 프로세스가 통째로 죽는다. 이유는 spawn이 이미 시스템 메시지로 알렸다.
    // 전달 기능이 생기면서 사용자가 안 보고 있는 방도 자동으로 깨우므로 실제로 걸린다.
    if (!this.child) return;
    ...
    if (!this._autoCompacting && !this._compactFailed && !this._awaitingTurn && this.contextTokens > this._compactAt()) {
      this._autoCompacting = true;
      this.queue = this.queue || [];
      this.queue.push({ text, images, opts });
      this.emit('message', this._bubble(text, images, opts, { pending: true }));
      this.emit('message', {
        role: 'system',
    ...
    if (this._autoCompacting) {
      this.queue = this.queue || [];
      this.queue.push({ text, images, opts });
      this.emit('message', this._bubble(text, images, opts, { pending: true })); // UI: "대기 중" 표시
      return;
    }
    this._writeUser(text, images, { emitMessage: true, midTurn: this._awaitingTurn, ...opts });
  }
  ...
  // 자동 압축을 실제로 거는 지점.
  //
  // 헤드리스(-p) 모드엔 TUI의 auto-compact가 없어서 여기서 대신 건다.
  //
  // 'limit'(기본)이면 한도의 92%. 절대값을 고른 방은 그 값을 쓰되 한도의 80%로 깎는다 —
  // 200k 모델에서 400k를 고르면 압축이 걸리기 전에 "Prompt is too long"으로 방이 통째로
  // 막히기 때문이다. 절대값 쪽에만 80%를 씌우는 이유는, 그건 "모델과 무관하게 이만큼"이라는
  // 뜻이라 한도를 모르고 고른 값이고, 'limit'은 한도를 알고 고른 값이라서다.
  _compactAt() {
    const picked = COMPACT_CHOICES.includes(this.room?.compactAt) ? this.room.compactAt : AUTO_COMPACT_AT;
    const limit = this._contextLimit();
    if (picked === 'limit') return Math.floor(limit * LIMIT_RATIO);
    return Math.min(picked, Math.floor(limit * 0.8));
  }
  ...
  _writeUser(text, images, { emitMessage, midTurn = false, peerFrom = null, wireSuffix = '' }) {
    if (!this.child) return; // 대기줄 투입 경로에서도 같은 크래시가 날 수 있다 (send 주석 참고)
    this._applyOptimisticModel(text);
    // 실제로 stdin에 쓰는 글과 말풍선에 보이는 글은 다를 수 있다.
    // 전달돼 온 말은 봉투를 씌워야 상대가 누가 보냈는지 알고, 전달 규칙 안내는
    // claude만 봐야 한다 (말풍선에 노출되면 대화가 지저분해진다).
    const wire = peerFrom
      ? wrapEnvelope(peerFrom, text)
      : wireSuffix
        ? `${text}\n\n${wireSuffix}`
        : text;
    const content = [
      ...
      ...(wire ? [{ type: 'text', text: wire }] : []),
    ];
    // stdin은 버퍼링되므로 init 전에 써도 안전함 (검증됨)
    this.child.stdin.write(
      JSON.stringify({ type: 'user', message: { role: 'user', content } }) + '\n'
    );
    if (emitMessage) this.emit('message', this._bubble(text, images, { peerFrom }));
  ...
  // 유휴 종료 직전에 거는 압축. 걸었으면 true (호출한 쪽은 끝날 때까지 기다렸다 죽인다).
  //
  // 깨울 때 값은 컨텍스트에 정비례한다 — resume은 대화 본문을 캐시에서 읽지 못하고 통째로
  // 다시 쓰고, 그 캐시가 1시간 TTL이라 쓰기 값이 정가의 2배다. 즉 300k 방을 그냥 내리면
  // 다음 한 마디가 600k다. 지금 압축하면 컨텍스트 1회 읽기(정가의 0.1배)로 그 값을 바닥까지
  // 낮춘다 — 300k 방 기준 600k → 100k.
  //
  // 시점을 여기로 옮긴 이유가 하나 더 있다. 압축은 몇 분씩 걸리는데, 임계에 걸려서 돌면
  // 사용자가 방금 친 말 앞에서 그 시간을 기다린다. 방치된 뒤에 돌면 아무도 안 기다린다.
  compactBeforeIdle() {
    if (!this.child) return false;
    if (this._autoCompacting || this._compactFailed || this._awaitingTurn) return false;
    if (this.state === 'working' || this.state === 'starting') return false;
    // 모델을 아직 못 봤으면 압축이 실패할 수 있다 (_compactAt 주석 참고)
    if (!this._lastModel) return false;
    // 바닥값 아래로는 못 내려가므로, 바닥의 2.5배는 넘어야 이득이다. 아직 바닥을 잰 적이
    // 없으면 실측 대략치(50k)를 쓴다 — 즉 125k 미만인 방은 그냥 내린다.
    const floor = this._floorTokens || 50_000;
    if (!this.contextTokens || this.contextTokens < floor * 2.5) return false;
    this._autoCompacting = true;
    this.emit('message', {
      role: 'system',
      text: `🗜️ 컨텍스트 ${Math.round(this.contextTokens / 1000)}k — 재우기 전에 압축할게요 (다음에 깨울 때 값을 줄이려고)`,
      ts: new Date().toISOString(),
    });
    this._writeUser('/compact', [], { emitMessage: false });
    return true;
...
module.exports = { HeadlessSession, COMPACT_CHOICES, AUTO_COMPACT_AT };
