...
// 800k는 TUI 기준(한도의 약 92%)에 _compactAt()의 안전 클램프(한도의 80%)를 씌운 값이다.
// 이 눈금이 없으면 "터미널에서 쓸 때랑 같게"를 아예 고를 수 없어서 넣었지만, 기본값으로
// 삼으면 안 된다. 캐시는 1시간 TTL이고 1시간 캐시 쓰기는 정가의 2배라, 방을 1시간 넘게
// 쉬었다 깨우는 첫 요청 하나가 "컨텍스트 × 2"를 문다. 실측(요청 10,133건)에서 이 재작성이
// 216건(2.1%)인데 비용의 25%였다 — 임계값을 800k로 올리면 그 25%가 4.4배가 된다.
// 방을 띄엄띄엄 쓰는 게 이 앱의 기본 사용 패턴이므로 낮은 임계가 유리하다.
const COMPACT_CHOICES = [140_000, 180_000, 250_000, 400_000, 800_000];
...
class HeadlessSession extends EventEmitter {
  ...
  // 방이 고른 값이 있으면 그걸 쓰되, 모델 한도의 80%는 절대 못 넘는다 — 넘기면 압축이
  // 걸리기 전에 "Prompt is too long"으로 방이 통째로 막힌다 (200K 모델에서 400k를 고른 경우).
  _compactAt() {
    const picked = COMPACT_CHOICES.includes(this.room?.compactAt) ? this.room.compactAt : AUTO_COMPACT_AT;
    return Math.min(picked, Math.floor(this._contextLimit() * 0.8));
  }
