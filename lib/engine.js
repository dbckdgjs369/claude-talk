...
class HeadlessSession extends EventEmitter {
  ...
  _onEvent(line) {
    ...
    if (ev.type === 'result') {
      ...
      } else if (!ev.is_error && this.contextTokens && this.contextTokens > this._contextLimit() * 0.8) {
        // TUI의 auto-compact 재현: 한도 80% 초과 시 선제 압축 (한도를 넘기 전에 해야 함)
        this._autoCompacting = true;
        this.emit('message', {
          role: 'system',
          text: `🗜️ 컨텍스트 ${Math.round((this.contextTokens / this._contextLimit()) * 100)}% — 자동 압축 중...`,
          ts: new Date().toISOString(),
        });
