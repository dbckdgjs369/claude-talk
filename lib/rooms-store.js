...
class RoomsStore {
  ...
  save() {
    const rooms = this.rooms.map(
      ({ id, dir, name, aiTitle, sessionId, lastPreview, lastTime, model, slashCommands, permissionMode,
         handle, compactAt, forkFrom, forkedFrom, forkedAt }) => ({
        id, dir, name, aiTitle, sessionId, lastPreview, lastTime, model, slashCommands, permissionMode,
        // 이 방이 고른 자동 압축 시점(토큰). 없으면 엔진 기본값을 쓴다
        compactAt,
        // 방 사이 말 전달에 쓰는 호출명(@handle). 방 제목과 분리해 두는 이유는
        // 제목은 AI가 지어 길고 바뀌는데, 멘션은 짧고 안 변해야 하기 때문
        handle,
        // 분기 정보. Claude Code는 계보를 안 남기므로(분기본에 원본 ID가 없다) 앱이 직접 기억한다
        forkFrom,   // 이어받을 원본 세션 ID (첫 턴에 자기 세션이 생기면 역할 종료)
        forkedFrom, // 원본 방 id — 목록 표시용
        forkedAt,   // 물려받은 메시지 개수 — 이 지점에 분기선을 긋는다
      })
    );
