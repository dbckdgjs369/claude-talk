...
function projectDirFor(cwd) {
  return path.join(process.env.HOME, '.claude', 'projects', encodeProjectDir(cwd));
}
  ...
  if (msg.role === 'assistant' && Array.isArray(msg.content)) {
    ...
    for (const c of msg.content) {
      if (c.type === 'text' && c.text && c.text.trim()) {
        ...
        out.push({ kind: 'msg', role: 'tool', text: summarizeToolUse(c), ts });
      }
    }
...
module.exports = { encodeProjectDir, projectDirFor, parseLine, parseFileSync, Tailer, findNewSessionFile, sessionFileFor, truncate };
